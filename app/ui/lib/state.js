...
export function addCue(showId, effectId, time, quantity = 1) {
  ...
  const cue = {
    id: "cue_" + Math.random().toString(36).slice(2, 9),
    effectId,
    time: Math.max(0, Math.min(show.duration, time)),
...
export function updateCue(showId, cueId, patch) {
  ...
  if (patch.time != null) {
    cue.time = Math.max(0, Math.min(show.duration, cue.time));
    show.cues.sort((a, b) => a.time - b.time);
  }
  show.updatedAt = Date.now();
  saveState();
...
export function showCost(show) {
  return show.cues.reduce((sum, c) => {
    const eff = getEffect(c.effectId);
...
function seedDemo(s) {
  const demo = {
    ...
    duration: 90,
    cues: [
      { id: "cue_d1", effectId: "font_30s_argent",     time: 0,  quantity: 2 },
      { id: "cue_d2", effectId: "com_30_argent",       time: 4,  quantity: 1 },
      { id: "cue_d3", effectId: "bomb_75_pivoine_rouge", time: 8,  quantity: 1 },
      { id: "cue_d4", effectId: "bomb_75_pivoine_bleue", time: 11, quantity: 1 },
      { id: "cue_d5", effectId: "cand_30_or_x12",      time: 15, quantity: 2 },
      { id: "cue_d6", effectId: "bomb_100_palmier",    time: 28, quantity: 1 },
      { id: "cue_d7", effectId: "bomb_100_chrysantheme", time: 35, quantity: 1 },
      { id: "cue_d8", effectId: "mine_75_multi",       time: 45, quantity: 2 },
      { id: "cue_d9", effectId: "bomb_125_couronne",   time: 55, quantity: 1 },
      { id: "cue_d10", effectId: "cand_30_multi_x20",  time: 62, quantity: 2 },
      { id: "cue_d11", effectId: "fin_20s_pack",       time: 75, quantity: 1 },
      { id: "cue_d12", effectId: "bomb_150_grand_finale", time: 88, quantity: 1 },
    ],
  };
  s.shows = [demo];
  return s;
